...
def scan_for_student_submissions(base_directory : str) -> list[str]:
  student_submissions_data = []
  print(f"Scanning for student submissions in '{base_directory}'...")

  # Iterate through each item in the base submission directory
  for item_name in os.listdir(base_directory):
      item_path = os.path.join(base_directory, item_name)

      # Check if the item is a directory (representing a student's folder)
      if os.path.isdir(item_path):
          student_name = item_name  # Use the directory name as the student's name
          submission_file_path = os.path.join(item_path, 'submission.txt')

          # Check if 'submission.txt' exists in the student's directory
          if os.path.exists(submission_file_path):
              student_submissions_data.append({
                  'student_name': student_name,
                  'submission_path': submission_file_path
              })
              print(f"Found submission for '{student_name}' at '{submission_file_path}'")
          else:
              print(f"No 'submission.txt' found for '{student_name}' in '{item_path}'")

  if student_submissions_data:
      print("\nIdentified student submissions:")
      for entry in student_submissions_data:
          print(f"- Student: {entry['student_name']}, Path: {entry['submission_path']}")
  else:
      print("No student submissions found matching the criteria.")

  return student_submissions_data